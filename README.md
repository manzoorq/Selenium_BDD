...
## Getting Started
- Clone the repo 'SeleniumCu_FW' to your local machine from commandline by typing `git clone <repo url>.
...
1. To run tests in parallel update the number of threads in the _./resources/testng.xml/_ file to number of desired tests you want to run in parallel. For example the  _thread-count_ is set to _2_ as depicted in the testng.xml file below. Make sure atleast 2 scenarios are available to run in the feature files as per the run tag configured in _TestRunner_ file.![image](https://github.com/user-attachments/assets/a1b5acfe-f1af-4135-8128-192894caa1e9)
2. Run the run test locally from IDE or from commandline `mvn clean test`. The tests should be triggered parallelly.

### Run from Jenkins(CI/CD)
1. Spin up Jenkins server in your system and create a maven style project.
...
The following is a non exhaustive list of the best practices and principles implemented in the framework-
1. The framework follows the **'cucumber maven project'** archetype.
   - The _test packages_ and _methods_ under src/test/java folder.
   - _Page classes, factory classes_ and utility methods under src/main/java folder.
...
7. Currently the framework supports **crossbrowser testing** via passing browser names as parameters in feature files and runing multiple features. There is a scope for future enhancements to add Browser and Platform factory class for enhanced cross browser and cross platform testing.
...
10. The Cucumber feature files have been written in Gherkin format best practices. The following cucumber traits have been implemented and exemplified in the _nyctimes.feature_ file
     - **Cucumber hooks** to initialize and closure activities.
     - **Scenario** with parameters.
     - **Scenario outlines** with Example tables data read from Excel.
     - Use of cucumber **tags** to control test selection for execution.
  11. The test runner files allows to configure the cucumber run time options like feature and step definition files and the type and output folder of reports that can be generated.
  12. The framework is **Data Driven** and supports
      ...
      - Data from Cucumber data tables, step parameters and Example tables.
  13. The code has properly described **code comments** especially for reviewers and test automation developers to understand the usage with ease.
  14. The code base follows the **Test Automation Principles**-
      - **Reusability** (**DRY**-Don't Repeat Yourself)- The DRY principle emphasises to avoid code duplication through. This is achieved via reusable steps definitions, page methods e.g. `User is on the Home page of <url>`.
      ...
      - **Scalability**- The parallel execution allows to run multiple tests on larger scale.
      - **Extensibility**- The utils packages can be enhanced with more util classes like DB connectors, screenshot utilities etc.
      - **KISS**(Keep it simple, stupid)- It emphasizes keeping code simple and avoiding unnecessary complexity. This help in easy debugging and troubleshooting. This framework writes simple and concise code.
      - **SRP**(Single Responsibility)- The PageFactory closely implements the SRP as if deals with the management of page objects and interactions.
...
4. Test Failure analyser and retry logics to handle flakiness.
